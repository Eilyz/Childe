...
export default {
  essentials: {
    'hubChannel': '929780583612157982',
    'parentCategory': '929780405530402836',
    'logsChannel': '929780551089553468',
  },
  ...
  request: async function (interaction) {
    ...
    const row = new MessageActionRow()
      .addComponents([
        new MessageButton()
          ...
          .setStyle('SUCCESS')
          .setLabel('Accept')
          .setEmoji('<:thumbsUp:862662918990069780>'),
        new MessageButton()
          .setCustomId('decline')
          .setStyle('DANGER')
          .setLabel('Decline')
          .setEmoji('<:ayakanotwao:879753878562832424>'),
      ])
    ...
    await hub.send({
      ...
      collector.on('collect', async i => {
        if (!i.member.roles.cache.has(Roles.adminRole)) return
  ...
  del: async function (interaction) {
    ...
    if (!interaction.member.roles.cache.has(Roles.adminRole))
      return interaction.editReply(`You can't delete a ticket due to permission requirements`)
    ...
    if (!user)
      return interaction.editReply(`Please provide an author of a ticket`)
